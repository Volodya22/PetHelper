...
namespace TestRest.Controllers
{
    public class PetController : ApiController
    {
            ...
            if (masterId != null && masterId != new Guid())
            {
                result =
                    _rep.Pets.Where(q => q.MasterId == masterId)
                        .Include("Master")
                        .Include("Breed.Type")
                        .Include("VaccinationsForPet")
                        .ToList();
            }
            else
            {
                result = _rep.Pets.Include("Master").Include("Breed.Type").Include("VaccinationsForPet").ToList();
            }
        ...
        public async Task<IHttpActionResult> PutPet(Guid id, Pet pet)
        {
            ...
            var vacsToRemove = _rep.PetsVaccinations.Where(v => v.PetId == pet.Id && !vacs.Contains(v.VaccinationId)).ToList();
            ...
            try
            {
                ...
                foreach (
                    var vac in
                        vacs.Where(
                            v =>
                                _rep.PetsVaccinations.FirstOrDefault(pv => pv.PetId == pet.Id && pv.VaccinationId == v) ==
                                null).ToList())
                {
                ...
                foreach (
                    var srv in
                        srvs.Where(
                            v => _rep.ServicesForPets.FirstOrDefault(p => p.PetId == pet.Id && p.ServiceId == v) == null)
                            .ToList())
                {
                    ...
                    if (srv == 4)
                    {
                        var n = _rep.Pets.Select(t => t.ChipId).Max();
                        pet.ChipId = n + 1;
                    }
                }
